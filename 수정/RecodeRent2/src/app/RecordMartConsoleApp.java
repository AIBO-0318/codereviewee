...
public class RecordMartConsoleApp {
	...
	String[] memberMenuList = {"로그아웃", "음반 목록", "음반 주문", "주문 목록", "장바구니 음반 담기", "장바구니 보기", "내 정보"};
	...
	private void displayWelcome() {
		System.out.println("*************************************");
		System.out.println("*  Welcome to Hyejeong Record Mart  *");
		System.out.println("*************************************");
	}
	...
	private void menuRecordRegist() {
		...
		String author = input.readString(">> 가수/아티스트 : ");
		String publisher = input.readString(">> 레이블 : ");
		int price = input.readInt(">> 가격 : ");
		int instock = input.readInt(">> 재고량 : ");
		
		if (bs.registRecord(new RecordVO(title, author, publisher, price, instock))) {
	...
	private void menuRecordRemove() {
		...
		if (confirm.equals(CONFIRM)) {
			if (bs.removeRecord(recordNo)) {
				System.out.println("[음반 삭제] 음반를 삭제했습니다.");
			} else {
